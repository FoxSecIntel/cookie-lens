...
<body>
  <div class="wrap">
    ...
    <footer class="footer">
      <div style="text-align: center; margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #2a2a3a; color: #666; font-size: 12px;">
        <p style="font-size: 16px; margin-bottom: 4px;">🍪 Cookie Lens</p>
        <p style="margin: 4px 0;">Protecting your privacy, one cookie at a time.</p>
        <p style="margin: 4px 0;">By <strong>FoxSecIntel</strong></p>
        <p style="margin: 4px 0;">
          <a href="https://github.com/FoxSecIntel/cookie-lens" style="color: #4caf82;">
            github.com/FoxSecIntel/cookie-lens
          </a>
        </p>
        <p style="margin-top: 8px; opacity: 0.6;">This report is for informational purposes only.</p>
      </div>
    </footer>
...
function generateTextSummary(data) {
  ...
  lines.push('─────────────────────────────────────');
  lines.push('🍪 Generated by Cookie Lens');
  lines.push('Protecting your privacy, one cookie at a time.');
  lines.push('By FoxSecIntel — https://github.com/FoxSecIntel/cookie-lens');
  lines.push('─────────────────────────────────────');
  return lines.join('\n');
}
...
async function copyToClipboard() {
  ...
  try {
    ...
    setTimeout(() => {
      btn.textContent = '📋';
      btn.title = 'Copy summary to clipboard';
      btn.disabled = false;
    }, 1500);
  } catch (err) {
    btn.textContent = '✗';
...
async function downloadReport() {
  ...
  try {
    ...
    setTimeout(() => {
      btn.textContent = '⬇';
      btn.title = 'Download HTML report';
      btn.disabled = false;
    }, 1500);
  } catch (err) {
    btn.textContent = '✗';
